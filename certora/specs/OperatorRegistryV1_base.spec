...
methods {
    ...
    //function _.pickNextValidatorsToDeposit(OperatorAllocation[]) external => DISPATCHER(true); // has no effect - CERT-4615
    ...
    function OR.pickNextValidatorsToDeposit(OperatorAllocation[]) external returns (bytes[] memory, bytes[] memory);
...
definition isMethodID(method f, uint ID) returns bool =
    ...
    (f.selector == sig:pickNextValidatorsToDeposit(OperatorAllocation[]).selector && ID == 7) ||
