...
///      upgrade time to the total LsETH ever requested at that moment, which is the end of the
///      pre-upgrade queue.
///
///      Rate marks never apply retroactively. A zero `RedeemRequestAnchor` already makes a
...
///      floor the first reports after the upgrade would spend their stopped-earning credit on
///      pre-upgrade requests that cannot use it, and the first post-upgrade cohort would be
///      short-changed by exactly that amount. The floor starts marking past them.
