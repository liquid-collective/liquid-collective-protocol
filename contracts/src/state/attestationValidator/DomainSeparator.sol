...
/// @notice Stores the EIP-712 domain separator used by the AttestationValidator,
///         computed once at initialisation time.
/// @dev    The cached separator binds `verifyingContract` to River's address (not the
///         validator's own address) so attester signing tooling can sign against
///         River's identity rather than a sibling validator that may be redeployed.
