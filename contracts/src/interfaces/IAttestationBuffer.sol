...
///         events for off-chain collection and lets a caller veto a deposit batch.
interface IAttestationBuffer {
    ...
    /// @param raiser The account that vetoed the batch (attribution is by msg.sender; the off-chain
    ...
    /// @notice Reverts when an attestation is submitted for a batch already vetoed.
    /// @param depositDataBufferId The vetoed batch identifier
    error BatchVetoed(bytes32 depositDataBufferId);

    /// @notice Submit an attestation for a deposit data batch.
    /// @dev Reverts `BatchVetoed` once the batch has been vetoed via `vetoBatch`.
    /// @param depositDataBufferId The hash of the deposit data being attested to.
    /// @param depositRootHash The deposit root hash at the time of attestation.
    ...
    /// @notice Veto a deposit batch as not ready for deposit ("unhappy path").
    /// @dev Open by design — attribution is by `msg.sender` and the off-chain backend filters to real
    ///      daemons/committee members. A veto is a sticky, hard on-chain stop: it makes further
    ...
    ///      deposits under a new nonce (which yields a fresh, unvetoed `depositDataBufferId`).
    ///      `errorCode`/`errorMessage` are a flexible, backend-defined payload. Re-vetoing an already-
    ///      vetoed id is allowed (emits again).
    /// @param depositDataBufferId The hash of the deposit data being vetoed.
    ...
    /// @notice Whether a deposit batch has been vetoed.
    /// @param depositDataBufferId The batch identifier
    /// @return True if the batch has been vetoed
