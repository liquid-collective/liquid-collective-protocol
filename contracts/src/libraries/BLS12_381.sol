...
import {LibUint256} from "./LibUint256.sol";
 ...
 * @author Lido
 * @author Solady
 * (https://github.com/Vectorized/solady/blob/dcdfab80f4e6cb9ac35c91610b2a2ec42689ec79/src/utils/ext/ithaca/BLS.sol)
...
library BLS12_381 {
     ...
     * @dev original at
     * https://github.com/Vectorized/solady/blob/dcdfab80f4e6cb9ac35c91610b2a2ec42689ec79/src/utils/ext/ithaca/BLS.sol#L275
     * @dev added comments and modified to use bytes32 instead of bytes and correct DST per
    ...
    function extractFlags(bytes1 componentHeaderByte) internal pure returns (bool signBit, bool areOtherFlagsValid) {
        // Binary structure of compressed component: [ compression flag(always 1), infinity flag(always 0), sign bit of
        // Y, ...rest of component ]
        uint8 componentHeader = uint8(componentHeaderByte);
        // extract sign bit via mask 0b00100000
        signBit = (componentHeader & 0x20) != 0;
        // extract compression & infinity flags via mask 0b11000000 and require the masked value to be 0x80 (top bits
        // '10')
    ...
    function validateCompressedSignatureFlags(bytes calldata signature, Fp2 calldata signatureY) internal pure {
        ...
        // ref:
        // https://github.com/ethereum/py_ecc/blob/05167bc2f11281a32cd18a8d4a7a7da6085be48d/py_ecc/bls/point_compression.py#L165
        ...
        // normal case, use c1 to determine sign bit similar to FP
            ...
            computedSignBit = signatureY.c1_a > HALF_P_A || (signatureY.c1_a == HALF_P_A && signatureY.c1_b > HALF_P_B);
        }
    ...
    function verifyDepositMessage(
        ...
        // pubkeyG1(pubkey | depositY.pubkeyY) | msgG2 | NEGATED_G1_GENERATOR | signatureG2(signature |
        // depositY.signatureY)
        ...
        // - signature is a 96-byte compressed G2 X coordinate (Fp2) encoded as `c1 || c0`, with flag bits in
        // signature[0]. - Pairing precompile expects X as `c0, c1` and each Fp limb as 64 bytes: 16 zero bytes ||
        // 48-byte limb.
        ...
        if (
            input[16] == 0 && input[17] == 0 && input[18] == 0 && input[19] == 0 && input[20] == 0 && input[21] == 0
                && input[22] == 0 && input[23] == 0
        ) {
            revert InputHasInfinityPoints();
        }
    ...
    function computeDepositDomain(bytes4 genesisForkVersion) internal view returns (bytes32 depositDomain) {
        bytes32 forkDataRoot = sha256Pair(bytes32(genesisForkVersion), bytes32(0));
        depositDomain = bytes32(DOMAIN_DEPOSIT_TYPE) | bytes32(uint256(forkDataRoot) >> 32);
    }
    ...
    function depositMessageSigningRoot(
        ...
        root = sha256Pair(
            ...
            sha256Pair(
                ...
                sha256Pair(
                    bytes32(LibUint256.toLittleEndian64(amount / 1 gwei)), // was: SSZ.toLittleEndian(amount / 1 gwei)
