...
// _updateFundedValidatorsFromBuffer logic (mirrors River.1.sol) so we can
...
contract AttestationDepositHarness is ConsensusLayerDepositManagerV1 {
    ...
    function _incrementFundedETH(uint256[] memory fundedETH) internal override {
...
contract ConsensusLayerDepositManagerAttestationTest is Test {
    ...
    function _makeDeposit(uint256 opIdx, uint256 seed)
        internal
        view
        ...
        returns (bytes32 bufferId, bytes32 rootHash, bytes[] memory sigs, BLS12_381.DepositY[] memory depositYs)
    {
        ...
        for (uint256 i = 0; i < deposits.length; i++) {
            depositYs[i] = BLS12_381.DepositY({pubkeyY: new bytes(0), sigY0: new bytes(0), sigY1: new bytes(0)});
        }
    }
    ...
    function testSuccessfulDeposit_depositRootAdvancesPerDeposit() public {
        ...
        (bytes32 bid1, bytes32 root1, bytes[] memory sigs1, BLS12_381.DepositY[] memory ys1) =
            _prepareDeposit(batch1);
        ...
        ys2[0] = BLS12_381.DepositY({pubkeyY: new bytes(0), sigY0: new bytes(0), sigY1: new bytes(0)});
    ...
    function testRevert_notKeeper() public {
        ...
        dm.depositToConsensusLayerWithAttestation(
            bytes32(0), bytes32(0), new bytes[](0), new BLS12_381.DepositY[](0)
        );
    }
    ...
    function testRevert_zeroWithdrawalCredentials() public {
        ...
        dm2.depositToConsensusLayerWithAttestation(
            bytes32(0), bytes32(0), new bytes[](0), new BLS12_381.DepositY[](0)
        );
    }
    ...
    function testRevert_insufficientAttestations() public {
        ...
        depositYs[0] = BLS12_381.DepositY({pubkeyY: new bytes(0), sigY0: new bytes(0), sigY1: new bytes(0)});
    ...
    function testRevert_staleDepositRoot() public {
        ...
        vm.prank(keeper);
        vm.expectRevert(
            abi.encodeWithSelector(DepositToConsensusLayerValidation.DepositRootMismatch.selector, staleRoot, actualRoot)
        );
