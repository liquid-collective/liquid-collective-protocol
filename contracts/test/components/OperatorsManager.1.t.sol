//SPDX-License-Identifier: MIT

pragma solidity 0.8.10;

import "../Vm.sol";
import "../../src/components/OperatorsManager.1.sol";
import "../../src/state/shared/AdministratorAddress.sol";

contract OperatorsManagerInitializableV1 is OperatorsManagerV1 {
    function publicOperatorsManagerInitializeV1(address _admin) external {
        AdministratorAddress.set(_admin);
    }

    function sudoSetFunded(string memory _name, uint256 _funded) external {
        Operators.Operator storage operator = Operators.get(_name);
        operator.funded = _funded;
    }
}

contract OperatorsManagerV1MemberManagementTests {
    Vm internal vm = Vm(0x7109709ECfa91a80626fF3989D68f67F5b1DD12D);

    OperatorsManagerV1 internal operatorsManager;
    address internal admin = address(0xEA674fdDe714fd979de3EdF0F56AA9716B898ec8);

    function setUp() public {
        operatorsManager = new OperatorsManagerInitializableV1();
        OperatorsManagerInitializableV1(address(operatorsManager)).publicOperatorsManagerInitializeV1(admin);
    }

    function testAddNodeOperator(address _nodeOperatorAddress, bytes32 _name) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _nodeOperatorAddress);
        Operators.Operator memory newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.operator == _nodeOperatorAddress);
    }

    function testAddNodeWhileNotAdminOperator(address _nodeOperatorAddress, bytes32 _name) public {
        vm.expectRevert(abi.encodeWithSignature("Unauthorized(address)", address(this)));
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _nodeOperatorAddress);
    }

    function testGetInexistingNodeOperator(bytes32 _name) public {
        vm.startPrank(admin);
        vm.expectRevert(abi.encodeWithSignature("OperatorNotFound(string)", string(abi.encodePacked(_name))));
        operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
    }

    function testSetOperatorAddressAsAdmin(
        bytes32 _name,
        address _firstAddress,
        address _secondAddress
    ) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);
        Operators.Operator memory newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.operator == _firstAddress);
        operatorsManager.setOperatorAddress(string(abi.encodePacked(_name)), _secondAddress);
        newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.operator == _secondAddress);
    }

    function testSetOperatorAddressAsOperator(
        bytes32 _name,
        address _firstAddress,
        address _secondAddress
    ) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);
        Operators.Operator memory newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.operator == _firstAddress);
        vm.stopPrank();
        vm.startPrank(_firstAddress);
        operatorsManager.setOperatorAddress(string(abi.encodePacked(_name)), _secondAddress);
        newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.operator == _secondAddress);
    }

    function testSetOperatorAddressAsUnauthorized(
        bytes32 _name,
        address _firstAddress,
        address _secondAddress
    ) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);
        Operators.Operator memory newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.operator == _firstAddress);
        vm.stopPrank();
        vm.startPrank(address(this));
        vm.expectRevert(abi.encodeWithSignature("Unauthorized(address)", address(this)));
        operatorsManager.setOperatorAddress(string(abi.encodePacked(_name)), _secondAddress);
    }

    function testSetOperatorStatusAsAdmin(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);
        Operators.Operator memory newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.active == true);
        operatorsManager.setOperatorStatus(string(abi.encodePacked(_name)), false);
        newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.active == false);
        operatorsManager.setOperatorStatus(string(abi.encodePacked(_name)), true);
        newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.active == true);
    }

    function testSetOperatorStatusAsUnauthorized(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);
        Operators.Operator memory newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.active == true);
        vm.stopPrank();
        vm.startPrank(address(this));
        vm.expectRevert(abi.encodeWithSignature("Unauthorized(address)", address(this)));
        operatorsManager.setOperatorStatus(string(abi.encodePacked(_name)), false);
    }

    function testSetOperatorStoppedValidatorCountAsAdmin(
        bytes32 _name,
        address _firstAddress,
        uint256 _stoppedCount
    ) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);
        Operators.Operator memory newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.stopped == 0);
        operatorsManager.setOperatorStoppedValidatorCount(string(abi.encodePacked(_name)), _stoppedCount);
        newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.stopped == _stoppedCount);
        operatorsManager.setOperatorStoppedValidatorCount(string(abi.encodePacked(_name)), 0);
        newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.stopped == 0);
    }

    function testSetOperatorStoppedValidatorCountAsUnauthorized(
        bytes32 _name,
        address _firstAddress,
        uint256 _stoppedCount
    ) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);
        Operators.Operator memory newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.stopped == 0);
        vm.stopPrank();
        vm.startPrank(address(this));
        vm.expectRevert(abi.encodeWithSignature("Unauthorized(address)", address(this)));
        operatorsManager.setOperatorStoppedValidatorCount(string(abi.encodePacked(_name)), _stoppedCount);
    }

    function testSetOperatorLimitCountAsAdmin(
        bytes32 _name,
        address _firstAddress,
        uint256 _limit
    ) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);
        Operators.Operator memory newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.limit == 0);
        operatorsManager.setOperatorLimit(string(abi.encodePacked(_name)), _limit);
        newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.limit == _limit);
        operatorsManager.setOperatorLimit(string(abi.encodePacked(_name)), 0);
        newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.limit == 0);
    }

    function testSetOperatorLimitCountAsUnauthorized(
        bytes32 _name,
        address _firstAddress,
        uint256 _limit
    ) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);
        Operators.Operator memory newOperator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(newOperator.limit == 0);
        vm.stopPrank();
        vm.startPrank(address(this));
        vm.expectRevert(abi.encodeWithSignature("Unauthorized(address)", address(this)));
        operatorsManager.setOperatorLimit(string(abi.encodePacked(_name)), _limit);
    }

    event AddedValidatorKeys(string indexed name, uint256 amount);

    function testAddValidatorsAsOperator(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        bytes
            memory tenPublicKeys = hex"";
        bytes
            memory tenSignatures = hex"";

        vm.stopPrank();
        vm.startPrank(_firstAddress);
        vm.expectEmit(true, true, true, true);
        emit AddedValidatorKeys(string(abi.encodePacked(_name)), 10);
        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);

        Operators.Operator memory operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 10);
    }

    function testAddValidatorsAsAdmin(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        bytes
            memory tenPublicKeys = hex"";
        bytes
            memory tenSignatures = hex"";

        vm.expectEmit(true, true, true, true);
        emit AddedValidatorKeys(string(abi.encodePacked(_name)), 10);
        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);

        Operators.Operator memory operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 10);
    }

    function testAddValidatorsAsAdminUnknownOperator(bytes32 _name) public {
        vm.startPrank(admin);

        bytes
            memory tenPublicKeys = hex"";
        bytes
            memory tenSignatures = hex"";

        vm.expectRevert(abi.encodeWithSignature("OperatorNotFound(string)", string(abi.encodePacked(_name))));
        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);
    }

    function testAddValidatorsAsUnauthorized(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        bytes
            memory tenPublicKeys = hex"";
        bytes
            memory tenSignatures = hex"";

        vm.stopPrank();
        vm.startPrank(address(this));
        vm.expectRevert(abi.encodeWithSignature("Unauthorized(address)", address(this)));
        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);
    }

    function testAddValidatorsInvalidPublicKeysSize(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        bytes
            memory tenPublicKeys = hex"";
        bytes
            memory tenSignatures = hex"";

        vm.expectRevert(abi.encodeWithSignature("InvalidPublicKeysLength()"));
        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);
    }

    function testAddValidatorsInvalidSignaturesSize(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        bytes
            memory tenPublicKeys = hex"";
        bytes
            memory tenSignatures = hex"";

        vm.expectRevert(abi.encodeWithSignature("InvalidSignatureLength()"));
        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);
    }

    function testAddValidatorsInvalidCount(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);
        vm.expectRevert(abi.encodeWithSignature("InvalidKeyCount()"));
        operatorsManager.addValidators(string(abi.encodePacked(_name)), 0, "", "");
    }

    function testRemoveValidatorsAsOperator(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        bytes
            memory tenPublicKeys = hex"";
        bytes
            memory tenSignatures = hex"";

        vm.stopPrank();
        vm.startPrank(_firstAddress);
        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);

        Operators.Operator memory operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 10);

        uint256[] memory indexes = new uint256[](10);

        indexes[0] = 9;
        indexes[1] = 8;
        indexes[2] = 7;
        indexes[3] = 6;
        indexes[4] = 5;
        indexes[5] = 4;
        indexes[6] = 3;
        indexes[7] = 2;
        indexes[8] = 1;
        indexes[9] = 0;

        operatorsManager.removeValidators(string(abi.encodePacked(_name)), indexes);
        operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 0);

        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);
        operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 10);
    }

    function testRemoveValidatorsAsAdmin(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        bytes
            memory tenPublicKeys = hex"";
        bytes
            memory tenSignatures = hex"";

        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);

        Operators.Operator memory operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 10);

        uint256[] memory indexes = new uint256[](10);

        indexes[0] = 9;
        indexes[1] = 8;
        indexes[2] = 7;
        indexes[3] = 6;
        indexes[4] = 5;
        indexes[5] = 4;
        indexes[6] = 3;
        indexes[7] = 2;
        indexes[8] = 1;
        indexes[9] = 0;

        operatorsManager.removeValidators(string(abi.encodePacked(_name)), indexes);
        operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 0);

        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);
        operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 10);
    }

    function testRemoveValidatorsUnauthorized(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        bytes
            memory tenPublicKeys = hex"";
        bytes
            memory tenSignatures = hex"";

        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);

        Operators.Operator memory operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 10);

        uint256[] memory indexes = new uint256[](10);

        indexes[0] = 9;
        indexes[1] = 8;
        indexes[2] = 7;
        indexes[3] = 6;
        indexes[4] = 5;
        indexes[5] = 4;
        indexes[6] = 3;
        indexes[7] = 2;
        indexes[8] = 1;
        indexes[9] = 0;

        vm.stopPrank();
        vm.startPrank(address(this));

        vm.expectRevert(abi.encodeWithSignature("Unauthorized(address)", address(this)));
        operatorsManager.removeValidators(string(abi.encodePacked(_name)), indexes);
    }

    function testRemoveValidatorsAndRetrieveAsAdmin(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        bytes
            memory tenPublicKeys = hex"";
        bytes
            memory tenSignatures = hex"";

        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);

        Operators.Operator memory operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 10);

        uint256[] memory indexes = new uint256[](1);

        indexes[0] = 0;

        (bytes memory pk, bytes memory s, ) = operatorsManager.getKey(string(abi.encodePacked(_name)), 9);

        operatorsManager.removeValidators(string(abi.encodePacked(_name)), indexes);
        operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 9);
        (bytes memory pkAfter, bytes memory sAfter, ) = operatorsManager.getKey(string(abi.encodePacked(_name)), 0);

        assert(keccak256(pkAfter) == keccak256(pk));
        assert(keccak256(sAfter) == keccak256(s));
    }

    function testRemoveValidatorsFundedKeyRemovalAttempt(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        bytes
            memory tenPublicKeys = hex"";
        bytes
            memory tenSignatures = hex"";

        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);

        Operators.Operator memory operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 10);

        uint256[] memory indexes = new uint256[](10);

        indexes[0] = 9;
        indexes[1] = 8;
        indexes[2] = 7;
        indexes[3] = 6;
        indexes[4] = 5;
        indexes[5] = 4;
        indexes[6] = 3;
        indexes[7] = 2;
        indexes[8] = 1;
        indexes[9] = 0;

        OperatorsManagerInitializableV1(address(operatorsManager)).sudoSetFunded(string(abi.encodePacked(_name)), 10);

        vm.expectRevert(abi.encodeWithSignature("InvalidFundedKeyDeletionAttempt()"));
        operatorsManager.removeValidators(string(abi.encodePacked(_name)), indexes);
    }

    function testRemoveValidatorsKeyOutOfBounds(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        bytes
            memory tenPublicKeys = hex"";
        bytes
            memory tenSignatures = hex"";

        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);

        Operators.Operator memory operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 10);

        uint256[] memory indexes = new uint256[](10);

        indexes[0] = 10;
        indexes[1] = 8;
        indexes[2] = 7;
        indexes[3] = 6;
        indexes[4] = 5;
        indexes[5] = 4;
        indexes[6] = 3;
        indexes[7] = 2;
        indexes[8] = 1;
        indexes[9] = 0;

        vm.expectRevert(abi.encodeWithSignature("InvalidIndexOutOfBounds()"));
        operatorsManager.removeValidators(string(abi.encodePacked(_name)), indexes);
    }

    function testRemoveValidatorsUnsortedIndexes(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        bytes
            memory tenPublicKeys = hex"";
        bytes
            memory tenSignatures = hex"";

        operatorsManager.addValidators(string(abi.encodePacked(_name)), 10, tenPublicKeys, tenSignatures);

        Operators.Operator memory operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.keys == 10);

        uint256[] memory indexes = new uint256[](10);

        indexes[0] = 9;
        indexes[1] = 1;
        indexes[2] = 7;
        indexes[3] = 6;
        indexes[4] = 5;
        indexes[5] = 4;
        indexes[6] = 3;
        indexes[7] = 2;
        indexes[8] = 1;
        indexes[9] = 0;

        vm.expectRevert(abi.encodeWithSignature("InvalidUnsortedIndexes()"));
        operatorsManager.removeValidators(string(abi.encodePacked(_name)), indexes);
    }

    function testGetOperatorByName(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        Operators.Operator memory operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        assert(operator.active == true);
    }

    function testGetOperatorByIndex(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);

        Operators.Operator memory operator = operatorsManager.getOperatorByName(string(abi.encodePacked(_name)));
        Operators.Operator memory operatorByIndex = operatorsManager.getOperator(0);
        assert(operator.active == true);
        assert(keccak256(bytes(operatorByIndex.name)) == keccak256(bytes(operator.name)));
    }

    function testGetOperatorCount(bytes32 _name, address _firstAddress) public {
        vm.startPrank(admin);
        assert(operatorsManager.getOperatorCount() == 0);
        operatorsManager.addOperator(string(abi.encodePacked(_name)), _firstAddress);
        assert(operatorsManager.getOperatorCount() == 1);
    }
}
