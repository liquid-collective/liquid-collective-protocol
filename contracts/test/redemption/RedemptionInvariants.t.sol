...
///      bug in `_findRateMarkAtOrBefore`'s predecessor search cannot be reproduced identically here
///      and cancel out.
abstract contract RedemptionMirror is RedemptionReportBase {
        ...
        returns (MirrorClaim memory result)
    {
        ...
        while (stepsLeft > 0 && remaining > 0 && eventId < eventCount) {
            ...
            result.steps += 1;
            // which side of `min(gross, cap)` decided this slice. Counted per slice, not per claim:
            // one claim routinely walks an over-funding event and an under-funding one.
            if (cap < gross) {
    ...
    /// @dev A rollback rather than a second deployment: with the whole protocol under test a second
    ///      world would mean a second River, Oracle, OperatorsRegistry and AttestationVerifier
    ///      redeployed once per fuzz run per world. The rollback gives the same starting point -- empty
    ///      queue, empty withdrawal stack, empty mark stack, floor at 0.
...
///      is either bounded into range or skipped explicitly, so the call counters measure work done
///      rather than calls attempted. Every skip bumps `calls_skipped`, so a run that bounced off the
///      guards is visible rather than merely quiet.
///
/// @dev RATE COHERENCE. `moveRate` is the only action that changes the pool rate, and every other
///      action prices itself against whatever rate is live when it runs. This is what makes the
///      sampled states reachable: River derives both legs of a stopped-earning report and both legs
///      of a withdrawal event from one rate (LibOracleReporting L217-219 and L588-613), so a handler
///      fuzzing a "mark rate" and a "settlement rate" independently of the pool would spend most of
///      its budget on unreachable states. It would also miss the case that matters most -- when the
///      three rates are within a few percent, `min(pro-rata event ETH, sliceCap)` is decided by which
///      truncation lands lower, whereas a 3x over-funded event makes the event side win
///      unconditionally. `capBoundSlices` and `eventBoundSlices` prove both sides occur.
contract RedemptionInvariantHandler is StdUtils {
    ...
    /// @notice Actions that bounced off a guard without reaching the protocol, including a claim that
    ///         found nothing in the queue currently satisfied by a withdrawal event.
    /// @dev Asserted to be zero over the deterministic sequence in
    ///      `test_HandlerActionsAreAllReachable`, so the counter has a consumer.
    ...
    function openRequest(uint256 userSeed, uint256 amountSeed) external {
        ...
        // Step 2: bound the redeemer and the size, then delegate. `_requestRedeem` anchors the request
        // at the live pool rate, so no rate is passed here.
        _test.handler_openRequest(bound(userSeed, 0, 2), bound(amountSeed, MIN_REQUEST, MAX_REQUEST));
        calls_openRequest++;
    }

    /// @notice Fuzzer entry point: lands an oracle report that only moves the pool rate. The sole
    ///         source of rate movement, which is what lets every other action price itself coherently.
    ...
    /// @dev CARVE-OUT: this handler never produces the zero-width withdrawal event
    ...
    ///      Panic(0x12) -- see
    ///      `RedemptionRoundingAndCapsTests.testZeroWidthWithdrawalEventBricksSpanningClaim`, which
    ///      owns the case. It is excluded because under `fail_on_revert = false` the resulting revert
    ///      would roll the whole handler call back and be invisible, which is worse than not sampling
    ///      it. This suite must not be read as evidence against that finding. Enforced on the funding
    ///      rather than the seed -- see `handler_reportWithdraw`.
    ...
    ///      bumped -- the test contract returns rather than reverts when nothing is satisfied.
    ///      `afterInvariant` asserts `calls_claim == ghost_claimCount`, which only holds if this
...
/// @dev The fixture starts with two pre-upgrade (anchor-less) requests already queued
...
///
/// @dev No payout floor is asserted here. Every cap property below is a one-sided ceiling -- I2 is
///      `paid <= capSum`, I9 is `received <= ethAtRequest + lockedAppreciation` -- so a mark whose
///      locked rate sits below a covered request's request-time rate, re-pricing that slice
///      downwards, satisfies all six invariants. That outcome is supported: a mark locks the rate in
///      force when the backing principal crossed exit_epoch, in both directions. The handler's rate
///      band is wide enough to sample it (`MIN_RATE`, `RATE_STEP_DOWN_BPS`), and the lower-bound
...
contract RedemptionInvariantsTest is RedemptionMirror {
    ...
    /// @dev Must not be refreshed per action, or `assertGe(floor, ghost_lastFloor)` in I8 compares the
    ///      floor to itself: `RateMarkFloor.set` has one call site, `initializeRedeemManagerV1_3`,
    ///      which runs in `setUp`, so both sides would always be equal and the monotonicity half of
    ///      that invariant could never fail.
    ...
    ///      `fail()` writes its flag outside the EVM journal -- but the diagnosis would be: it
    ///      surfaces as `[FAIL: <empty revert data>]` on every invariant at once, with the message
    ///      gone. Recording a delta and asserting it from a named `invariant_` points at one
    ///      property.
    function setUp() public override {
        ...
        // from the fuzzer's first call. Without it a run must draw open -> withdrawal report -> claim
        // in that order before a wei is paid, and a run that misses the ordering satisfies all six
        // invariants vacuously with `ghost_claimCount == 0`. That was the common case, not a corner.
        // Partial on purpose, 5 of the 17 ether: it leaves `RedeemDemand` non-zero so `reportWithdraw`
        // still finds work, and request 0 half-settled so the legacy pro-rata branch is live.
    ...
    /// @dev The push relies on ids staying dense and sequential, so `ghost_endPositions[i]` is the
    ///      record for request `i`. Not asserted here -- an in-frame assertion would lose its message
    ///      per the note on `setUp`, and `invariant_RequestEndPositionIsImmutable` catches any
    ///      divergence via its length check.
    /// @dev No rate is passed: `_requestRedeem` anchors the request at the live pool rate, which makes
    ///      the anchor comparable to the mark and settlement rates that follow.
    ...
    /// @dev The buffer is a per-report staging area, not a running total, so the conservation identity
    ///      needs what River has already taken back as its own term.
    /// @dev A report can only drain the buffer -- nothing on the report path adds to it, only a claim
    ///      does -- so the drop across the call measures the pull exactly, and stays correct if the APR
    ///      headroom ever bounds it to a partial one.
    ...
    /// @dev The call goes through `_tryClaim` so that a claim which resolved to a real event and then
    ///      reverted stays visible: an uncaught revert would unwind this frame, taking the ghost writes
    ///      and the call counter with it, and `fail_on_revert = false` would still report green.
    ///      Reverts land in `ghost_claimReverted`, asserted from `afterInvariant`.
    /// @dev Returns whether a claim executed, so `calls_claim` means exactly that. Both early returns
    ///      are outcomes the fuzzer should see as skips: nothing satisfied, or the claim reverted.
    function handler_claim(uint256 idSeed, uint16 depth) external returns (bool claimed) {
    ...
    /// @dev Under `fail_on_revert = false` a claim landing on an unsatisfied request wastes one of the
    ///      run's 32 actions: it returns without touching the protocol. Redirecting those draws is what
    ///      makes the claim action reliably do work. The seed still chooses among the satisfied
    ///      requests, so the interleaving the fuzzer explores is preserved.
    ...
    /// @dev Kept out of `handler_claim` so that function's stack stays inside the limit.
    ...
    ///      not needed: the span the claim consumed already encodes where the walk stopped. Kept out of
    ///      `handler_claim` to keep that function's stack inside the limit.
    ...
    /// Scenario: any interleaving of the five handler actions.
    /// Expected: every wei a withdrawal event supplied is accounted for once -- paid to a recipient or
    /// parked in the exceeding-eth buffer -- and once an event's whole LsETH span has been claimed,
    /// at most one wei per matched slice is left unaccounted.
    /// Why it matters: the exceeding-eth buffer is the only sink for the difference between what an
    /// exit returned and what a redeemer is owed. If the two sides do not add up, either a redeemer
    /// was overpaid out of someone else's exit or ETH is stranded with no owner.
    function invariant_ConservationPerWithdrawalEvent() public {
        ...
        // global form: every wei a withdrawal event supplied is paid to a recipient, still staged in
        // the exceeding-eth buffer, or already reclaimed into River by a later report
        assertEq(
    ...
    /// Expected: the ETH a claim delivers never exceeds the summed caps of the slices it was assembled
    /// from, and it matches an independently derived payout wei for wei.
    /// Why it matters: the cap is the feature. It turns "the redeemer keeps what their stake earned in
    /// the queue" into an enforceable ceiling; without it a rich event would pay every request the
    ...
    /// Why it matters: withdrawal events, rate marks and the next request's start position are all
    /// located relative to that end position, so a claim that moved it would silently re-point every
    /// downstream lookup.
    ...
    /// Expected: marks are strictly ascending, pairwise disjoint, and the last ends at or below the
    /// total LsETH ever requested.
    /// Why it matters: `_findRateMarkAtOrBefore` is a predecessor binary search, correct only on a
    /// sorted non-overlapping stack. An overlap would let two marks claim the same LsETH and pay a
    /// redeemer the higher of the two locked rates.
    ...
    /// `totalRequestedHeight`. If either overshot, `markable` would be computed against a position no
    /// request occupies and marks would be issued against demand that does not exist.
    ...
    /// Expected: the floor never decreases, and no mark starts below it.
    /// Why it matters: the floor stops the first post-upgrade reports from spending their credit on
    /// pre-upgrade requests that cannot use it. A mark below the floor is silently burnt credit -- the
    /// first post-upgrade cohort short-changed by that amount, with no error anywhere.
    ...
    /// @dev `ghost_claimCount > 0` is not asserted, for an arithmetic reason. The fuzzer draws
    ///      uniformly across five selectors, so a 32-call run never calls `claim` with probability
    ///      (4/5)^32 ~= 0.08% -- roughly one campaign in ten across 128 runs, which is a flaky CI job
    ///      rather than a guard, and Foundry offers no cross-run aggregation. Vacuity is attacked by
    ///      construction instead: `setUp` seeds a withdrawal event so a claim pays on the first call,
    ///      and `_firstResolvableFrom` redirects draws that would land on an unsatisfied request. What
    ///      is asserted is the part that cannot be flaky: the claim counter matches the claims that ran.
    /// @dev The two `min(gross, cap)` branch counters are not asserted here either -- a single 32-call
    ///      run need not walk the rate far enough in both directions, and campaigns where the cap
    ///      never decided a payout do occur. Both sides are proven reachable deterministically in
    ///      `test_HandlerActionsAreAllReachable`, which establishes that the branch can be hit, not
    ///      that a given stateful run hit it.
    function afterInvariant() public {
        ...
        // the claim counter must agree with the claims that executed. This is what keeps
        // `calls_total()` honest -- it is the only evidence a run has that it did any work.
    ...
    /// ETH on others, and all six invariants hold.
    /// Why it matters: two blind spots close here. `fail_on_revert = false` means a handler that
    /// reverted on every call would still report 128 green runs, so each action needs a standing proof
    /// of reachability. And a suite that only over-funds its events makes the cap the binding side of
    /// every payout, so the event-side truncation is never the deciding term.
    /// @dev The rate walk is written against the handler's step bounds: `moveRate` draws from
    ///      `[current * 0.7, current * 1.1]`, so each literal below sits inside the window its
    ///      predecessor opens and `bound` returns it unchanged. 1.0 -> 1.1 -> 1.21 appreciates;
    ///      1.21 -> 0.847 is a 30% slash, which puts a settlement rate below a request rate and makes
    ///      the event the binding side.
    function test_HandlerActionsAreAllReachable() external {
        ...
        // 60 ETH of principal stops earning at the live 1.1. The mark opens at the floor of 17 ether,
        // since everything below it is pre-upgrade demand.
        ...
        // the two legacy (anchor-less) requests sit first on the axis, so a withdrawal event settles
        // them first: claiming id 0 exercises the legacy cap branch, and at a settlement rate of 1.21
        // against a request-time rate of 1.0 the cap binds
        handler.claim(0, 8);
        ...
        // id 2 is anchored and partly marked, so it exercises the slice-cap branch, and it straddles
        // the two events, so one slice is capped at 1.21 and the other under-funded at 0.847
        handler.claim(2, 8);
        ...
        // the counter has to mean "a claim executed", not "a claim was attempted". Every action here
        // was chosen to do real work, so nothing may have bounced off a guard either.
        ...
        // both sides of `min(gross, cap)` were exercised, which is why the rate walks down as well as up
        assertGt(ghost_capBoundSlices, 0, "no slice was decided by the cap");
...
/// @dev Extends the fuzz coverage in contracts/test/RedeemManager.1.t.sol rather than restating it:
///      `testFuzz_MarkedRequestPayoutRespectsCapAndConservesEth` pins the single-mark single-event
///      case and `testFuzz_SplitClaimNeverPaysMoreThanWholeClaim` an inequality across differing
///      event geometry. Everything below is a property those do not reach.
/// @dev The two worlds a differential property needs are two states of one deployment, separated by a
///      snapshot rollback. See `RedemptionMirror._resetToPristineProtocol`.
contract RedemptionRateMarkFuzzTests is RedemptionMirror {
    ...
    /// @dev Created in `setUp` rather than per run so it survives the snapshot rollback: an allowlist
    ///      grant issued after the snapshot would be undone and the second world's request revert.
    ...
    function setUp() public override {
        ...
        // An empty queue at upgrade time pins the floor at 0, so every request a scenario below opens
        // is anchored and fully markable.
        _upgradeToV1_3();
    ...
    /// @dev The pool rate is moved before each action and every leg derived from it, rather than each
    ///      action being handed its own rate. That is what keeps the geometry reachable: River computes
    ///      a mark's locked rate and an event's settlement rate from the single live rate, so a mark
    ///      priced at 2.0 while the pool sits at 1.0 is unreachable. Each rate move is its own report,
    ///      standing for however many it took the pool to walk there.
    /// @dev The last event settles the remaining demand rather than a computed third. A report funds
    ///      the RedeemManager in ETH and River converts back with `sharesFromUnderlyingBalance`, so the
    ///      round trip `floor(floor(lsETH * rate) / rate)` can lose a wei per event and three thirds
    ///      would leave the request short of exhausted. Over-funding the last event puts
    ///      `_reportWithdrawToRedeemManager` on its full-demand branch, where it clamps to the demand
    ///      exactly and skims the excess back to the deposit buffer.
    function _buildScenario(Scenario memory s) internal returns (RedeemManagerV1 manager, uint32 id) {
        ...
        // not written back into `s`: the struct is memory, so a caller replaying the scenario onto a
        // second world would otherwise pass the first world's opened amount as the target and build a
        // geometry a wei or two different
    ...
    /// Scenario: the identical request, mark stack and event geometry claimed twice on two pristine
    /// protocol states -- once in a single unbounded call, once in K calls whose recursion depths are
    /// fuzzed independently until the request is exhausted.
    /// Expected: the two totals are equal wei for wei, for every K and every depth sequence.
    /// Why it matters: `_depth` is the escape hatch for a request pending across more oracle reports
    /// than fit in one transaction. If splitting the walk changed the payout by a wei, that escape
    /// hatch would carry a price and an old request would be worth less than a new one purely because
    /// of how it had to be claimed. `testFuzz_SplitClaimNeverPaysMoreThanWholeClaim` asserts only
    /// `<=`, which is all that holds across differing event geometry; holding the geometry fixed and
    /// splitting only the walk is the stronger statement.
    function testFuzz_SplitDepthClaimPaysExactlyTheWholeClaim(
        ...
        Scenario memory s;
        s.user = fuzzUser;
        // at least a gwei, so each of the three events settles a span that survives the ETH round trip
        ...
        uint256 steps = 0;
        // three events means at most three steps; the bound is a liveness guard, and the assertion
        // below proves the loop finished the request rather than timing out
        ...
        // buffer instead of to the recipient
        assertEq(
    ...
    /// protocol states -- one where a mark covers its entire span, one with no marks at all.
    ...
    /// that physically overlaps its positions. This is the test that `anchor.lsETHAtRequest == 0` is a
    /// hard cutover and not merely a different default.
    function testFuzz_LegacyRequestPayoutIgnoresRateMarks(
        ...
        // each rate move is its own report, landed before the action it prices, so the mark's locked
        // rate and the event's settlement rate are both ones River derives
        ...
        // the same fuzzed target against the same pristine state at the same rate, so both worlds
        // carry the same position to the wei
    ...
    /// Scenario: an anchored request settled by a single over-funded withdrawal event, with a fuzzed
    /// fraction of its span marked at a fuzzed locked rate.
    /// Expected: the ETH the request receives over its lifetime never exceeds its request-time value
    /// plus the appreciation the marks covering it locked in.
    /// Why it matters: this is the economic property the stopped-earning feature exists to guarantee.
    /// A redeemer should keep the yield their stake earned while it sat in the exit queue, and stop
    /// earning at the moment a native staker's principal would have. If a redeemer could be paid more
    /// than `ethAtRequest + locked appreciation` they would out-earn a native staker at the expense of
    /// everyone still in the pool, funded out of exceeding-eth that would otherwise return to River.
    /// @dev The settlement rate is derived as a small step above `max(requestRate, markRate)` rather
    ///      than drawn from a band of its own. Both halves matter:
    ///        - above the larger cap rate, so the cap stays the binding side of
    ///          `min(pro-rata event ETH, sliceCap)` on every run. An under-funding event satisfies the
    ///          ceiling trivially and would leave the property untested.
    ///        - by a small step, because a band over-funding every event by 3x makes `gross >= cap`
    ///          hold by construction on every input, so the two truncations never compete. A margin of
    ///          0-10% puts them back in contention.
    ///      It is also the only reachable version: the settlement rate is the pool rate at the sweeping
    ///      report, so an event at 6.0 after a mark locked 3.0 describes a pool that doubled between
    ///      two reports.
    function testFuzz_RedeemerNeverOutEarnsNativeStaker(
        ...
        if (markedAmount > 0 && applyRate(markedAmount, markRate) > 0) {
            ...
            if (redeemManager.getRateMarkCount() > 0) {
                ...
                // read the LsETH leg back rather than reusing `markedAmount`: River floors it with
                // `sharesFromUnderlyingBalance`, so the mark can be a wei narrower
        ...
        // and the ceiling is the binding constraint here, not the event's ETH
    ...
    /// Expected: the payout equals the independently derived per-slice
    /// `min(pro-rata event ETH, slice cap)` sum wei for wei, and never exceeds the summed caps.
    /// Why it matters: the direct, stateless form of the cap ceiling over the geometry that exercises
    /// all three branches of the `_sliceCap` walk -- gap, covered range, stale predecessor mark -- in
    /// a single claim.
    function testFuzz_ClaimNeverExceedsSliceCapAcrossMarkGaps(
        ...
        // mark [0, quarter). Marking resumes at the cursor, so to leave a gap the second mark must be
        // pushed after a withdrawal event has advanced the settled height past it -- the "settled from
        // the deposit buffer, never exited" case the RateMarkStack header names as the source of
        // permanent gaps. Each action is preceded by the rate move that prices it.
    ...
    /// Why it matters: the lower-bound half of the cap, which nothing else in this suite asserts. I2
    /// and I9 are both ceilings, so a mark that re-prices a slice downwards is invisible to them. It
    /// is a supported outcome: a mark locks the rate in force when the backing principal crossed
    /// exit_epoch, in both directions, so a redeemer marked during a drawdown forfeits any later
    /// recovery on the marked span, a `CoverageFundV1` payout included. Pinned here so the forfeiture
    /// cannot change silently in either direction -- this test fails if a floor at the request-time
    /// rate is introduced, as does
    /// `SliceCapGeometryTests.testMarkBelowRequestRateRePricesSliceDownwards`.
    /// @dev The settlement rate is derived as a small step above the request rate, for the same reason
    ///      `testFuzz_RedeemerNeverOutEarnsNativeStaker` derives its own: the cap has to be the binding
    ///      side in both worlds, or the comparison degenerates into two identically under-funded
    ///      events. Above the request rate, world 2 is pinned at `ethAtRequest` and world 1 at the
    ///      mark, so the gap between them is the forfeited recovery and nothing else.
    ...
    function testFuzz_MarkBelowRequestRateForfeitsRecovery(
        ...
        // the control is held at its request-time value: the drawdown alone costs it nothing, since
