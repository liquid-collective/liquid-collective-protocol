...
contract RedeemQueueMigrationV1_2 is Test {
    ...
    function test_migrate_allRequestInOneCall() external shouldSkip {
        ...
        RedeemQueueV1.RedeemRequest[] memory redeemRequestsOld= new RedeemQueueV1.RedeemRequest[](redeemRequestCount);
        for (uint32 i = 0; i < redeemRequestCount; i++) {
            redeemRequestsOld[i] = RedeemManager.getRedeemRequestDetails(i);
        }

        // Set up the fork at a new block for testing the upgrade
        vm.createSelectFork(_rpcUrl, 2359200);
        ...
        for (uint32 i = 0; i < redeemRequestCount; i++) {
            ...
            assertEq(newRequest.amount, redeemRequestsOld[i].amount);
            assertEq(newRequest.maxRedeemableEth, redeemRequestsOld[i].maxRedeemableEth);
            assertEq(newRequest.recipient, redeemRequestsOld[i].recipient);
            assertEq(newRequest.height, redeemRequestsOld[i].height);
        }
    }
}