...
*Uses OZ ERC20 _burn to disallow burning from address(0).Decreases the total supply.*
...
*Alias for BurnFrom for compatibility with the older naming convention.Uses burnFrom for all validation &amp; logic.*
...
```solidity
function grantMintAndBurnRoles(address burnAndMinter) external nonpayable
```

grants both mint and burn roles to `burnAndMinter`.

*calls public functions so this function does not require access controls. This is handled in the inner functions.*

#### Parameters

| Name | Type | Description |
|---|---|---|
| burnAndMinter | address | undefined |
...
```solidity
function initialize(string _name, string _symbol, address _admin) external nonpayable
```
...
*Uses OZ ERC20 _mint to disallow minting to address(0).Disallows minting to address(this)Increases the total supply.*
...
```solidity
function setCCIPAdmin(address newAdmin) external nonpayable
```
